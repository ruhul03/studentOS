...
- **UI & Design System Overhaul**:
  ...
  - `Mobile Layout & Scroll Fixes`: Disabled nested internal scrolling (`h-full`, `overflow-y-auto`) exclusively on mobile for complex pages (`StudyPlanner`, `StudentMarketplace`, `EventsAnnouncements`) to enable smooth, native browser scrolling. Fixed a critical "horizontal squish" layout bug by injecting `min-w-0` into the root `DashboardLayout` flex container, preventing wide child components (like the 800px Planner calendar) from implicitly scaling out the entire mobile viewport width.
